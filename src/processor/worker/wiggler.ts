...
const applyWiggle = (base: OffscreenCanvas, mask: OffscreenCanvas, contours: readonly Vector2D[][], normalRadiansForContours: readonly number[][], eps: number, movableLength: number, std: number, isStride: boolean): OffscreenCanvas => {
  ...
  for(const contourIdx in contours) {
    ...
    for(const iStr in contour) {
      ...
      patchCtx.drawImage(base, 
        pos[0] - size, pos[1] - size, mask.width, mask.height,
